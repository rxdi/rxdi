...
export class FormGroup<T = FormInputOptions, E = { [key: string]: never }> {
 ...
 private updateValueAndValidityOnEvent(method: Function) {
  ...
  return async function (this: AbstractInput, event: { target: AbstractInput }) {
   ...
   const hasMultipleBindings = [
    ...((self.getFormElement().querySelectorAll(selector) as any) as Map<
     string,
     NodeListOf<Element>
    >).values(),
   ].length;
 ...
 private querySelectAll(name: string) {
  return [
   ...((<never>this.form.querySelectorAll(name)) as Map<string, AbstractInput>).values(),
  ];
 }
   ...
   return {
    errors: errors.concat(
     Object.keys(InputValidityState)
      .map((key) =>
       element.validity[key] ? { key, message: element.validationMessage } : null,
      )
      .filter((i) => !!i),
    ),
    element,
 ...
 private async mapInputErrors(element: AbstractInput) {
  const res = await Promise.all(
   (this.validators.get(element.name) || []).map(async (v) => {
