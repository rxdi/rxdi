import { Component, classMap, html, LitElement, property } from '@rxdi/lit-html';
import creditCardType from 'credit-card-type';

import { CardTypes } from './images';
import { convertCard } from '../helpers';
import { CardCSS } from './credit-card-component.css';
...
@Component({
 ...
 styles: [CardCSS],
 template(this: CreditCardComponent) {
  return html`
   <div class="container" @click=${() => (this.flipped = !this.flipped)}>
    <div class=${classMap({ creditcard: true, flipped: this.flipped })}>
     <div class="front">
      <div id="ccsingle">
       ${CardTypes[creditCardType(convertCard(`${this.number}`))?.[0]?.type]}
      </div>
      <svg
       version="1.1"
       id="cardfront"
...
export class CreditCardComponent extends LitElement {
