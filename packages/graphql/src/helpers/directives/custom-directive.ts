...
const GraphQLCustomDirectivePrivate: any = function(config) {
 ...
 */
export const applySchemaCustomDirectives: (
  schema: GraphQLSchema
) => GraphQLSchema = schema => {
  if (!(schema instanceof GraphQLSchema)) {
    throw new Error('Schema must be instanceof GraphQLSchema');
  }
  wrapFieldsWithMiddleware(schema.getQueryType());
  wrapFieldsWithMiddleware(schema.getMutationType(), false);
  return schema;
};
...
export const GraphQLCustomDirective: GraphQLCustomDirective = GraphQLCustomDirectivePrivate;

export interface GraphQLCustomDirective {
  new <T = {}, K = {}, A = {}>(
    options: GraphQLCustomDirectiveInterface<T, K, A>
  ): GraphQLDirective;
}
